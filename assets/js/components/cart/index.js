...
const cartIndex = (parent) => {
    cartData.data.forEach(item => {
        ...
        el.querySelector('button#remove-button').addEventListener('click', (e) => {
            e.preventDefault();
            cartData.delete(product.id);
            el.remove(); // Entfernt das Element aus dem DOM
        });

        el.querySelector('button#increment-button').addEventListener('click', (e) => {
            e.preventDefault();
            let input = el.querySelector('input#counter-input');
            let value = input.value;
            value++;
            input.value = value;
            cartData.update(product.id, value);
        });

        el.querySelector('button#decrement-button').addEventListener('click', (e) => {
            e.preventDefault();
            let input = el.querySelector('input#counter-input');
            let value = input.value;
            if (value > 1) value--;
